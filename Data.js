...
(async function html_scraper() { 
	...
	if (symbolLink) {
		...
		companyData.push({
			...
			'EarningsPerShare(trailing)': additionalDataArray[4],
			'EarningsPerShare(future)': additionalDataArray[5],
			'DividendYield': additionalDataArray[6],
			'Dividend': additionalDataArray[7],
	...
	async function html_scraper2(Link) { 
			...
			const allRows2 = $$('table.table-sm.table-borderless > tbody > tr');
				...
				console.log("loading:", dataValues[1]);
